...
def list_pixel_formats() -> dict[str, dict[str, bool | int | str]]:
    ...
    if (
        ...
        for f in _pixel_formats:
            ...
            storage_bpp: int = max(list(map(int, bit_depths.split('-'))))
            pixel_formats[k] = {
                'c': int(nc),
                'bpp': storage_bpp,
                'pipe_bpp': int(bpp),
...
if False:
    for k, v in PIXEL_FORMATS.items():
        if v['supported']:
            print(lightgreen(f"{k:<10}"), f"\tc_order={v['c_order']}, bpp={v['bpp']}, pipe_bpp={v['pipe_bpp']}")
        else:
            print(darkgrey(f"{k:<10}\tbpp={v['bpp']}, pipe_bpp={v['pipe_bpp']}"))
